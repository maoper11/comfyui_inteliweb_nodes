...
function isStateValue(value) {
  ...
  try {
    ...
    return parsed && typeof parsed === "object" && Array.isArray(parsed.loras);
  } catch {
    return false;
  }
}

function legacyStateWidget(node) {
  return node.widgets?.find((widget) => widget?.name === HIDDEN_INPUT);
}

function removeLegacyArtifacts(node) {
  const widget = legacyStateWidget(node);
  if (widget) {
    const index = node.widgets?.indexOf(widget) ?? -1;
    if (index >= 0) node.widgets.splice(index, 1);
    for (const element of [widget.element, widget.inputEl, widget.el]) element?.remove?.();
  }

  for (let index = (node.inputs?.length || 0) - 1; index >= 0; index -= 1) {
    const input = node.inputs[index];
    if (input?.name === HIDDEN_INPUT || input?.widget?.name === HIDDEN_INPUT) {
...
function readNodeState(node) {
  ...
  if (!isStateValue(raw)) raw = legacyStateWidget(node)?.value;
...
function migrateSerializedNode(nodeData) {
  if (!nodeData || typeof nodeData !== "object") return;
  const type = nodeData.type ?? nodeData.comfyClass;
  if (type !== NODE_CLASS) return;

  nodeData.properties ||= {};
  if (!isStateValue(nodeData.properties[STATE_PROP])) {
    const values = Array.isArray(nodeData.widgets_values) ? nodeData.widgets_values : [];
    const legacy = values.find(isStateValue);
    nodeData.properties[STATE_PROP] = serializeState(parseState(legacy));
  }

  // The old node had exactly one serializable widget: the internal JSON STRING.
  // The new DOM widget uses serialize:false, so no widget value should remain.
  if (Array.isArray(nodeData.widgets_values)) {
    nodeData.widgets_values = nodeData.widgets_values.filter((value) => !isStateValue(value));
  }

  if (Array.isArray(nodeData.inputs)) {
    nodeData.inputs = nodeData.inputs.filter(
      (input) => input?.name !== HIDDEN_INPUT && input?.widget?.name !== HIDDEN_INPUT,
    );
  }
}

function migrateWorkflowData(root) {
  const seen = new WeakSet();
  const walk = (value) => {
    if (!value || typeof value !== "object" || seen.has(value)) return;
    seen.add(value);
    migrateSerializedNode(value);
...
function injectStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
.inteliweb-lora-stack{display:grid;gap:4px;width:100%;min-width:${MIN_CONTENT_WIDTH}px;box-sizing:border-box;padding:1px 2px 0;overflow:visible!important;font:12px ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color:#e8e8e8}
.inteliweb-lora-stack *{box-sizing:border-box}
.inteliweb-lora-header{display:grid;grid-template-columns:minmax(0,1fr) max-content;gap:4px}
.inteliweb-lora-summary,.inteliweb-lora-row{border:1px solid #4d4d4d;background:#292929;border-radius:6px}
.inteliweb-lora-summary{display:flex;align-items:center;height:27px;padding:2px 5px}
.inteliweb-lora-rows{display:grid;gap:4px}
.inteliweb-lora-row{display:grid;grid-template-columns:30px minmax(24px,1fr) 58px 24px;gap:3px;align-items:center;min-height:33px;padding:3px;background:#252525}
.inteliweb-lora-row.disabled{opacity:.55}
.inteliweb-lora-empty{padding:8px;border:1px dashed #555;border-radius:6px;color:#999;text-align:center}
.inteliweb-lora-button,.inteliweb-lora-icon{border:1px solid #555;border-radius:5px;background:#202020;color:#eee;cursor:pointer;font-weight:600}
.inteliweb-lora-button:hover,.inteliweb-lora-icon:hover{background:#353535}
.inteliweb-lora-add{height:27px;padding:0 10px;background:#111;color:#fff;white-space:nowrap}
.inteliweb-lora-icon{width:24px;height:25px;padding:0;font-size:15px}
.inteliweb-lora-switch{display:inline-flex;align-items:center;gap:6px;cursor:pointer;user-select:none}
.inteliweb-lora-switch.compact{justify-content:center;gap:0}
.inteliweb-lora-switch input{position:absolute;opacity:0;pointer-events:none}
.inteliweb-lora-track{position:relative;width:30px;height:16px;border:1px solid #666;border-radius:999px;background:#3a3a3a}
.inteliweb-lora-track:after{content:"";position:absolute;top:2px;left:2px;width:10px;height:10px;border-radius:50%;background:#bbb;transition:transform .12s ease}
.inteliweb-lora-switch input:checked+.inteliweb-lora-track{background:#666}
.inteliweb-lora-switch input:checked+.inteliweb-lora-track:after{transform:translateX(14px);background:#fff}
.inteliweb-lora-picker-trigger{display:grid;grid-template-columns:minmax(0,1fr) 12px;align-items:center;width:100%;height:25px;padding:2px 5px;border:1px solid #505050;border-radius:5px;background:#191919;color:#f0f0f0;cursor:pointer;text-align:left}
.inteliweb-lora-picker-value{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.inteliweb-lora-picker-value.missing{color:#ffcc80}
.inteliweb-lora-strength{display:grid;grid-template-columns:14px minmax(20px,1fr) 14px;height:25px;border:1px solid #505050;border-radius:5px;background:#191919;overflow:hidden}
.inteliweb-lora-strength button{border:0;background:transparent;color:#aaa;cursor:pointer;padding:0;font-size:8px}
.inteliweb-lora-strength button:hover{background:#303030;color:#fff}
.inteliweb-lora-strength input{width:100%;min-width:0;border:0;outline:0;background:transparent;color:#f0f0f0;text-align:center;appearance:textfield;-moz-appearance:textfield}
.inteliweb-lora-strength input::-webkit-inner-spin-button,.inteliweb-lora-strength input::-webkit-outer-spin-button{appearance:none;-webkit-appearance:none}
.inteliweb-lora-popover{position:fixed;z-index:100000;display:grid;grid-template-rows:max-content minmax(0,1fr);min-width:240px;max-width:calc(100vw - 16px);max-height:min(420px,calc(100vh - 16px));overflow:hidden;border:1px solid #666;border-radius:7px;background:#171717;color:#f0f0f0;box-shadow:0 12px 32px rgba(0,0,0,.55)}
.inteliweb-lora-search-row{display:grid;grid-template-columns:minmax(0,1fr) max-content;gap:8px;padding:7px;border-bottom:1px solid #404040;background:#202020}
.inteliweb-lora-search{min-width:0;padding:6px 8px;border:1px solid #5a5a5a;border-radius:5px;background:#111;color:#fff}
.inteliweb-lora-options{overflow:auto;padding:4px}
.inteliweb-lora-option{display:block;width:100%;padding:6px 8px;border:0;border-radius:4px;background:transparent;color:#ededed;cursor:pointer;text-align:left;overflow-wrap:anywhere}
.inteliweb-lora-option:hover,.inteliweb-lora-option.selected{background:#3b3b3b}
...
function positionPicker(trigger, panel) {
  ...
  panel.style.left = `${Math.round(Math.min(Math.max(margin, rect.left), window.innerWidth - width - margin))}px`;
  const below = window.innerHeight - rect.bottom - margin;
  const above = rect.top - margin;
  const height = Math.min(panel.scrollHeight || 420, 420, window.innerHeight - margin * 2);
  panel.style.top = `${Math.round(below < 220 && above > below ? Math.max(margin, rect.top - height - 4) : Math.min(rect.bottom + 4, window.innerHeight - height - margin))}px`;
}
...
function makePicker(current, onChange) {
  ...
  trigger.addEventListener("click", () => {
    ...
    const render = () => {
      ...
      const names = terms.length ? all.filter((name) => terms.every((term) => name.toLowerCase().includes(term))) : all;
...
function fitNode(node) {
  ...
  const widget = node.widgets?.find((item) => item.name === "lora_stack_ui");
  const widgetTop = Number(widget?.last_y);
  const top = Number.isFinite(widgetTop) && widgetTop > 0 ? widgetTop : 96;
  const height = Math.max(MIN_NODE_HEIGHT, Math.ceil(top + root.offsetHeight + 8));
  node.__inteliwebLoraDesiredHeight = height;
  if (Math.abs((node.size?.[1] || 0) - height) > 1) node.setSize?.([width, height]);
...
function renderNode(node) {
  ...
  state.loras.forEach((row, index) => {
    ...
    const openMenu = (event) => {
      ...
      new LiteGraph.ContextMenu([
        { content: row.on === false ? "🟢 Enable" : "⚫ Disable", callback: () => { row.on = row.on === false; writeNodeState(node); renderNode(node); } },
        null,
        { content: "⬆️ Move Up", disabled: index === 0, callback: () => move(-1) },
        { content: "⬇️ Move Down", disabled: index === state.loras.length - 1, callback: () => move(1) },
        { content: "🗑️ Remove", callback: () => { state.loras.splice(index, 1); writeNodeState(node); renderNode(node); } },
      ], { title: "LORA", event });
    };
  ...
  add.addEventListener("click", async () => {
    ...
    try {
      ...
      state.loras.push({ on: true, name: available[0] || "", strength: 1, strength_model: 1, strength_clip: 1 });
      writeNodeState(node);
      renderNode(node);
    } catch (error) {
...
function prepareNode(node) {
  ...
  if (!node.__inteliwebLoraRoot) {
    ...
    const widget = node.addDOMWidget?.("lora_stack_ui", "INTELIWEB_LORA_STACK", root, {
      serialize: false,
      hideOnZoom: false,
      getMinHeight: () => root.offsetHeight || 64,
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onResize = function (size, ...args) {
      if (Array.isArray(size)) {
        ...
        if (Number.isFinite(this.__inteliwebLoraDesiredHeight)) size[1] = this.__inteliwebLoraDesiredHeight;
      }
      return typeof originalResize === "function" ? originalResize.call(this, size, ...args) : undefined;
    };

    const originalRemoved = nodeType.prototype.onRemoved;
    nodeType.prototype.onRemoved = function (...args) {
      if (activePicker?.trigger && this.__inteliwebLoraRoot?.contains(activePicker.trigger)) closePicker();
      return typeof originalRemoved === "function" ? originalRemoved.apply(this, args) : undefined;
    };
  },
...
// Pixaroma-style execution bridge: state is stored on node.properties and only
// injected into the backend's truly hidden input when graphToPrompt is built.
